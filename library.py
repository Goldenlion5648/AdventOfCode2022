...
from math import ceil, exp, floor, gcd, factorial, inf
import math
# math.
...
class AdventInput:
    ...
    def setup(self, data):
        ...
        if len(self.lines) == 1:
            self.line = data
            self.word = data
        if all(x.isnumeric() for x in self.lines):
...
def debug(*args, **kwargs):
    # print(sys.argv)
    if len(sys.argv) == 1 or ("d" in sys.argv or "debug" in sys.argv) and "real" not in sys.argv and "r" not in sys.argv:
...
def to_dict(a):
    ...
    if isinstance(a, str):
        a = a.split("\n")
    return {(y, x) : a[y][x] for y in range(len(a)) for x in range(len(a[y]))}
...
def show_board(board, actual_symbol=True, filler='.', max_y=None, max_x=None, conversions=None, ending=''):
    ...
    lowest_x = min(j[1] for j in board)
    lowest_y = min(j[0] for j in board)
    # print(f"conversions: {conversions}")
    for y in range(lowest_y, y_dim):
